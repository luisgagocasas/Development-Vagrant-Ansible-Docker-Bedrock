...
if [ ! -d "$BEDROCK_FOLDER" ];
  then
    ...
    cd bedrock
    wp dotenv set DB_NAME $WORDPRESS_DB_NAME --allow-root
    wp dotenv set DB_USER root --allow-root
    wp dotenv set DB_PASSWORD $WORDPRESS_DB_PASSWORD --allow-root
    wp dotenv set DB_PREFIX $WORDPRESS_DB_PREFIX --allow-root
